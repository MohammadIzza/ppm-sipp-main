...
class C_admin_usulan extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('logged_in')) {
            ...
            $data['ppm'] = $this->m_data->getTbbycond2("usulan, usulananggota, peneliti", "usulan.idusulan=usulananggota.idusulan && usulananggota.nipnim=peneliti.nipnim && usulananggota.posisi='Ketua' $setperiode $setskema $setstatus", "usulan.idusulan", "desc");
            ...
            $this->load->view('v_admin_usulan', $data);
        } else {
    ...
    public function export() {
        if ($this->session->userdata('logged_in')) {
            ...
            $excel->setActiveSheetIndex(0)->setCellValue('A1', "DAFTAR USULAN PENELITIAN DAN PENGABDIAN"); // Set kolom A1 dengan tulisan "DATA SISWA"
            ...
            $usulan = $this->m_data->getTbbycond2("usulan, usulananggota, peneliti, periode, skema", "usulan.idusulan=usulananggota.idusulan && usulananggota.nipnim=peneliti.nipnim && usulananggota.posisi='Ketua' && usulan.idperiode=periode.idperiode && usulan.idskema=skema.idskema $setperiode $setskema $setstatus", "usulan.idusulan", "desc");
            ...
            foreach ($usulan as $data) { // Lakukan looping pada variabel siswa
                ...
                $excel->setActiveSheetIndex(0)->setCellValue('C' . $numrow, $data->gelardepan.' '.$data->namalengkap.' '.$data->gelarblkg);
