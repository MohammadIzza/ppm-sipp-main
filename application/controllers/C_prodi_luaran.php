...
class C_prodi_luaran extends CI_Controller {
    ...
    public function index()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $data['usulan']=$this->m_data->getTbbycond2("usulan, usulananggota, peneliti", "usulan.idusulan=usulananggota.idusulan and usulananggota.nipnim=peneliti.nipnim and usulan.statususulan='Didanai' && peneliti.idprodi='$idprodi' group by usulan.idusulan", "usulan.idusulan", "desc");
            $data['ppm']=$this->m_data->getTbbycond2("usulanfile, jenisfile, usulan", "usulanfile.idjenis=jenisfile.idjenis and jenisfile.kategori='luaran' and usulanfile.idusulan=usulan.idusulan and usulan.statususulan='Didanai' $setperiode $setjenis and usulan.idusulan in(select idusulan from usulananggota where nipnim in(select nipnim from peneliti where idprodi='$idprodi'))", "usulanfile.idfile", "desc");
            ...
            $this->load->view('v_prodi_luaran',$data);
        }else{
